                    ...
                    if get_response.status_code == 200:
                        ...
                        if current_cluster:
                            ...
                            if total_in > 0:
                                ...
                                in_response = requests.post(f"{FLASK_API_URL}/clusters/{cluster_info['id']}/movement", json=in_data)
                                
                            if total_out > 0:
                                out_data = {"movement_type": "OUT", "quantity": total_out}
                                out_response = requests.post(f"{FLASK_API_URL}/clusters/{cluster_info['id']}/movement", json=out_data)
                            ...
                            if movement_success:
                                ...
                                if st.button("Retry Update"):
                                    ...
                                    retry_success = True
                                    if total_in > 0:
                                        in_response = requests.post(f"{FLASK_API_URL}/clusters/{cluster_info['id']}/movement", json={"movement_type": "IN", "quantity": total_in})
                                        if in_response.status_code not in (200, 201):
                                            retry_success = False
                                    
                                    if total_out > 0:
                                        out_response = requests.post(f"{FLASK_API_URL}/clusters/{cluster_info['id']}/movement", json={"movement_type": "OUT", "quantity": total_out})
